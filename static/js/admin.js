...
const TAB_META = {
  ...
  reviews: { title: "Reviews", sub: "Manage the client reviews shown on the public site.", addLabel: "Add Review" },
...
const ENTITY = {
  ...
  reviews: {
    ...
    listItem: (item) => ({
      thumb: item.image,
      title: item.name,
      sub: item.designation ? `${item.designation}${item.company ? ' · ' + item.company : ''}` : (item.company || ''),
      tag: item.rating ? `${item.rating}/5` : "",
    }),
    fields: [
      { name: "name", label: "Name", type: "text", required: true },
      { name: "designation", label: "Designation", type: "text" },
      { name: "company", label: "Company", type: "text" },
      { name: "rating", label: "Rating", type: "number", hint: "1-5", row: "rating" },
